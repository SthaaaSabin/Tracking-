...
// ═══════════════════════════════════════════════════════════════════════════════
//  DEMO MODE — TEMPORARY LOCAL MOCK TRACKER
// ═══════════════════════════════════════════════════════════════════════════════
//
//  WHY THIS EXISTS:
//    Meta Business account is currently restricted → Meta Pixel cannot be set up.
//    This mock tracker simulates the exact same event flow locally so you can
//    demo and test the full engagement tracking concept without a real pixel.
//
//  WHAT IT DOES:
//    • Saves every tracked event to localStorage (key: "demo_pixel_events")
//    • Logs each event to the browser console
//    • Updates the floating Demo Panel on the page in real time
//
//  HOW TO SWAP BACK TO REAL META PIXEL (when account is restored):
//    1. Set  DEMO_MODE = false  below
//    2. Uncomment the Meta Pixel <script> in layout.tsx
//    3. Replace 'PLACEHOLDER_PIXEL_ID' with your real Pixel ID
//    4. The trackEvent() and trackCustomEvent() calls throughout this file
//       will automatically route to fbq() instead of the local mock.
//
//  EVENT FLOW (same as real pixel would track):
//    Facebook/Instagram post or ad
//      → user clicks website link
//      → lands on this page
//      → tracking fires: PageView, LearnMoreClick, SupportInterest, LeadSubmitted
//
// ═══════════════════════════════════════════════════════════════════════════════

const DEMO_MODE = true                    // ← Set to false when using real Meta Pixel
const STORAGE_KEY = 'demo_pixel_events'   // localStorage key for demo events

// ─── Shape of a single tracked event ─────────────────────────────────────────
interface DemoEvent {
  id:        string
  name:      string
...
// ─── Global callback so the panel can react to new events ────────────────────
//     (simple pub/sub without a full state library)
let onNewEvent: (() => void) | null = null

// ─── Core tracker — called everywhere in this file ────────────────────────────
//
...
const trackEvent = (name: string, meta?: Record<string, unknown>) => {
  ...
  if (DEMO_MODE) {
    ...
    writeStoredEvents([...existing, event])

    // ── Log to console so devs / demo viewers can see it ──
    console.log(
      `%c[DEMO PIXEL] %c${name}`,
      'color:#1e4a8f; font-weight:bold;',
      'color:#44403c; font-weight:normal;',
      meta ?? ''
    )

    // ── Notify the Demo Panel to re-render ──
    onNewEvent?.()

  } else {
    // ── LIVE: route to real Meta Pixel ──
...
// For custom Meta Pixel events (trackCustom vs track) — same mock logic
const trackCustomEvent = (name: string, meta?: Record<string, unknown>) => {
  if (DEMO_MODE) {
    trackEvent(name, meta) // mock treats custom & standard the same
  } else {
    if (typeof window !== 'undefined' && typeof window.fbq === 'function') {
...
// Extend window type for Meta Pixel (used in live mode)
declare global {
...
// Event name → accent colour for the Demo Panel badges
const EVENT_COLORS: Record<string, string> = {
  PageView:       'bg-navy-100 text-navy-700',
  LearnMoreClick: 'bg-gold-100 text-gold-700',
  SupportInterest:'bg-emerald-50 text-emerald-700',
  LeadSubmitted:  'bg-purple-50 text-purple-700',
}
const EVENT_DEFAULT_COLOR = 'bg-stone-100 text-stone-600'

// ═══════════════════════════════════════════════════════════════════════════════
//  DEMO PANEL COMPONENT
//  A small floating panel that shows every locally-tracked event in real time.
//  This is for demo/testing ONLY — not part of the real campaign site.
// ═══════════════════════════════════════════════════════════════════════════════
...
function DemoPanel() {
  ...
  // Read initial events from localStorage
  useEffect(() => {
    setEvents(readStoredEvents())
  }, [])

  // Subscribe to new events via global callback
  useEffect(() => {
    onNewEvent = () => {
      setEvents(readStoredEvents())
      setJustAdded(true)
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-2">
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="w-80 bg-white rounded-2xl shadow-card-lg border border-stone-100 overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-stone-100 bg-stone-50">
              <div className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                <p className="text-[13px] font-semibold text-stone-700">Demo Tracker</p>
                <span className="text-[11px] text-stone-400 font-normal">· local only</span>
              </div>
              <span className="text-[12px] font-medium text-navy-600 bg-navy-50 px-2 py-0.5 rounded-full">
            ...
            <div className="max-h-64 overflow-y-auto divide-y divide-stone-50">
              {events.length === 0 ? (
                <p className="text-stone-400 text-[13px] text-center py-6">
                ...
                [...events].reverse().map((ev, i) => (
                  ...
                  >
                    <span className={`shrink-0 mt-0.5 text-[11px] font-semibold px-2 py-0.5 rounded-full ${EVENT_COLORS[ev.name] ?? EVENT_DEFAULT_COLOR}`}>
                      {ev.name}
                    </span>
                    <span className="text-stone-400 text-[11px] mt-0.5 tabular-nums">
            ...
            {/* Footer */}
            <div className="px-4 py-3 border-t border-stone-100 bg-stone-50 flex items-center justify-between">
              <p className="text-stone-400 text-[11px] leading-snug">
                Simulates Meta Pixel events.<br />
                Stored in <code className="font-mono">localStorage</code>.
              </p>
              <button
                onClick={handleClear}
                className="text-[11px] font-medium text-red-400 hover:text-red-600 transition-colors ml-3 shrink-0"
              >
                Clear
              </button>
            </div>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        className={`
          flex items-center gap-2 px-4 py-2.5 rounded-full shadow-card-lg
          text-[13px] font-semibold transition-all duration-200
          ${open
            ? 'bg-navy-800 text-white'
            : 'bg-white text-navy-700 border border-stone-200'}
        `}
      >
        {/* Pulse dot — blinks briefly when new event fires */}
        <motion.span
          animate={justAdded ? { scale: [1, 1.8, 1] } : {}}
          transition={{ duration: 0.4 }}
          className={`w-2 h-2 rounded-full ${justAdded ? 'bg-emerald-400' : open ? 'bg-white/50' : 'bg-emerald-400'}`}
        />
        Demo Tracker
        {events.length > 0 && (
          <span className={`text-[11px] px-1.5 py-0.5 rounded-full font-bold
            ${open ? 'bg-white/20 text-white' : 'bg-navy-100 text-navy-600'}`}>
            {events.length}
          </span>
        )}
        <svg
...
export default function LandingPage() {
  ...
  //  In real Meta Pixel: fbq('track', 'PageView')
  //  In demo mode: stored to localStorage and logged to console.
  // ─────────────────────────────────────────────────────────────────────────
  useEffect(() => {
    trackEvent('PageView', { source: 'direct_or_social' })
  }, [])
  ...
  // ─────────────────────────────────────────────────────────────────────────
  //  TRACKING: LearnMoreClick (Custom Event)
  //  Fires when visitor clicks "Learn More" in the hero.
  //  Signals curiosity / top-of-funnel engagement.
  // ─────────────────────────────────────────────────────────────────────────
  const handleLearnMore = () => {
    scrollTo('about')
    trackCustomEvent('LearnMoreClick', { location: 'hero', button: 'Learn More' })
  }

  // ─────────────────────────────────────────────────────────────────────────
  //  TRACKING: SupportInterest (Custom Event)
  //  Fires when visitor clicks "I'm Interested" — high intent signal.
  // ─────────────────────────────────────────────────────────────────────────
  const handleInterestedClick = () => {
    scrollTo('involved')
    trackCustomEvent('SupportInterest', { location: 'hero', button: "I'm Interested" })
  }
  ...
  const validate = () => {
    ...
    if (!formData.consent) e.consent = 'Please check the consent box to continue.'
    setErrors(e)
    return Object.keys(e).length === 0
  }

  // ─────────────────────────────────────────────────────────────────────────
  //  TRACKING: LeadSubmitted (Conversion Event)
  //  Fires on successful form submission.
  //  This is the primary conversion event — most important for ad optimisation.
  //  In real Meta Pixel: fbq('track', 'Lead') + fbq('trackCustom', 'LeadSubmitted')
  // ─────────────────────────────────────────────────────────────────────────
  ...
  return (
    <div className="min-h-screen bg-cream-50 overflow-x-hidden">
      ...
      >
        <div className="max-w-6xl mx-auto flex items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <motion.span animate={mobileMenuOpen ? { rotate: 45, y: 6 }  : { rotate: 0, y: 0 }}
                className="block w-5 h-0.5 bg-stone-700 origin-center" />
              <motion.span animate={mobileMenuOpen ? { opacity: 0 } : { opacity: 1 }}
      ...
      <section id="hero" className="relative min-h-screen flex items-center pt-20 overflow-hidden">
        ...
        />
        <motion.div className="absolute top-[15%] left-[10%] w-[500px] h-[500px] bg-navy-400/10 rounded-full blur-3xl pointer-events-none"
          animate={{ x: [0,20,0], y: [0,-15,0] }}
          transition={{ duration: 18, repeat: Infinity, ease: 'easeInOut' }} />
        <motion.div className="absolute bottom-[10%] right-[5%] w-[400px] h-[400px] bg-gold-400/8 rounded-full blur-3xl pointer-events-none"
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 w-full py-16 md:py-24">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            {/* Text */}
            <motion.div initial="hidden" animate="visible" variants={stagger(0.14)}>
              ...
              <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-4">
                {/* ── trackEvent('LearnMoreClick') fires on click ── */}
                <motion.button
                  id="hero-learn-more"
                ...
                {/* ── trackEvent('SupportInterest') fires on click ── */}
                <motion.button
          ...
          >
            <div>
              ...
              <motion.h2 variants={fadeUp}
                ...
                Rooted in This<br /><span className="italic">Community</span>
              </motion.h2>
              <motion.div variants={fadeUp} className="space-y-5 text-[17px] font-body text-stone-600 leading-[1.85]">
                ...
                <p>
                  This campaign is not about a title. It is about carrying forward a lifetime of
                  work — and ensuring the next generation inherits a district they are proud to call home.
                </p>
              </motion.div>
            ...
            <motion.div variants={fadeUp} className="space-y-6">
              <div className="relative bg-navy-900 text-white rounded-2xl p-8 md:p-10 shadow-card-lg overflow-hidden">
                ...
                <p className="font-body italic text-xl md:text-2xl leading-relaxed text-cream-200 relative z-10">
                  &ldquo;I don&apos;t believe in politics as usual. I believe in sitting at your kitchen
                  table, hearing your worries, and going back to the chamber to do something about them.&rdquo;
                </p>
                <p className="mt-5 text-gold-400 font-medium text-sm relative z-10">— James Hargrove</p>
              </div>
              {[
                { emoji: '🏡', title: 'Born & Raised Here',    text: "Not a career politician transplanted from the capital. James grew up here and his family still lives here." },
                { emoji: '🤝', title: 'Non-partisan Record',   text: "James has consistently worked across lines to get things done — because good policy doesn't have a party colour." },
              ].map(card => (
                <div key={card.title} className="flex items-start gap-4 bg-white rounded-2xl p-6 shadow-card border border-stone-100">
                  ...
                  <div>
                    ...
                    <p className="text-stone-500 text-[15px] leading-relaxed font-body">{card.text}</p>
                  </div>
                </div>
              ))}
            </motion.div>
        ...
        <div className="max-w-6xl mx-auto px-6">
          ...
          >
            <motion.div variants={fadeUp} className="text-center max-w-2xl mx-auto mb-16">
              <span className="text-navy-500 font-medium text-sm uppercase tracking-[0.18em]">Priorities</span>
            ...
            <div className="grid md:grid-cols-3 gap-7">
              {PRIORITIES.map(p => (
                ...
                >
                  ...
                  <h3 className="font-serif text-xl text-navy-900 mb-3 leading-snug">{p.title}</h3>
                  <p className="text-stone-500 text-[15px] leading-relaxed font-body">{p.body}</p>
      ...
      <section id="vision" className="py-24 md:py-32 bg-navy-950 relative overflow-hidden">
        <div className="absolute inset-0 opacity-[0.05]"
          style={{ backgroundImage: "url(\"data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23ffffff' fill-opacity='1'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E\")" }}
        />
        <motion.div className="absolute -top-32 -right-32 w-96 h-96 bg-navy-600/20 rounded-full blur-3xl"
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
          ...
          >
            <motion.span variants={fadeUp} className="text-gold-400 font-medium text-sm uppercase tracking-[0.2em]">
              Our Vision
            </motion.span>
            <motion.h2 variants={fadeUp}
              className="font-serif text-4xl md:text-5xl xl:text-6xl text-white mt-4 mb-8 leading-tight">
              A District That Works<br /><span className="italic text-cream-300">for Everyone</span>
            </motion.h2>
            <motion.div variants={fadeUp} className="ornament-divider mb-8">
            ...
            <motion.button
              variants={fadeUp} id="vision-get-involved"
      ...
      {/* ═══════════════════════════════════════════════════════
          INTEREST FORM
          trackEvent('LeadSubmitted') fires on successful submit
      ══════════════════════════════════════════════════════ */}
          ...
          <div className="grid lg:grid-cols-2 gap-14 items-start">
            ...
            >
              ...
              <motion.h2 variants={fadeUp}
                ...
                Your Voice<br /><span className="italic">Matters Here</span>
              </motion.h2>
              <motion.p variants={fadeUp}
                  ...
                  {formSubmitted ? (
                    ...
                    >
                      ...
                      <p className="text-stone-500 font-body text-[16px] leading-relaxed max-w-sm mx-auto">
                        Your message has been received. A member of James&apos;s team will be in
                        touch soon. We&apos;re glad you&apos;re with us.
                      </p>
                    </motion.div>
                    ...
                    >
                      ...
                      <div>
                        ...
                        {errors.name && <p className="mt-1.5 text-[13px] text-red-500">{errors.name}</p>}
                      </div>
                      ...
                      <div>
                        ...
                        <input id="form-phone" type="tel" autoComplete="tel"
                      ...
                      <div className="pt-1">
                        <label className="custom-checkbox flex items-start gap-3 cursor-pointer">
                          <input id="form-consent" type="checkbox"
                      ...
                      {/* Submit — trackEvent('LeadSubmitted') fires inside handleSubmit */}
                      <motion.button
                        id="form-submit" type="submit" disabled={isSubmitting}
